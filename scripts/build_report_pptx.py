#!/usr/bin/env python3
"""Chạy pipeline preserve-template-scaffold để sinh report.pptx an toàn."""
...
TEMPLATE = BASE / "format_template.pptx"
TARGET = BASE / "report.pptx"
PIPELINE = BASE / ".opencode/skills/md-to-pptx-pipeline/scripts"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Chạy pipeline build PPTX an toàn theo contract preserve-template-scaffold.")
    parser.add_argument("--run-dir", default=str(BASE / ".office-auto/state/manual-run-pptx"))
    parser.add_argument("--source-file", default=str(BASE / "slide_content.md"))
    ...
    run_step("build_pptx.py", "--run-dir", str(run_dir))
    run_step("qa_pptx.py", "--run-dir", str(run_dir))
