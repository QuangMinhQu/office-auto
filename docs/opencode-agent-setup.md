...
- [.vscode/mcp.json](../.vscode/mcp.json): share OfficeCLI MCP server theo kiểu workspace-local config, để repo tự mang theo tool registration cần thiết.
...
- OfficeCLI MCP là execution path native phù hợp nhất với repo hiện tại vì không phá contract `preserve-template-scaffold`.
...
- Nếu sau này cài `uvx`, có thể thêm một server review/redline riêng bên cạnh `officecli` thay vì thay thế nó.
