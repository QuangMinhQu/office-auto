...
## Primitive flow bắt buộc
1. Gọi `docx_pipeline_inspectTemplate` để lấy `template_inspection_raw.json`.
...
5. Gọi `docx_pipeline_validateExecutionOps`.
6. Nếu có warnings, sửa `execution_ops.json` rồi validate lại.
7. Gọi `docx_pipeline_applyExecutionOps`.
8. Gọi `docx_pipeline_readResult`.
