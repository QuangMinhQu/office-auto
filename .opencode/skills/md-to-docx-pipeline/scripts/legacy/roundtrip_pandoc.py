...
def build_roundtrip_report(
    ...
    *,
    ...
    return {
        ...
        "style_spec_used": style_spec_used,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Roundtrip output DOCX qua Pandoc để làm semantic QA.")
    ...
    if build_report.get("status") != "completed" or not target_file.exists():
        report = {
            ...
            "message": "Build chưa hoàn tất nên chưa thể roundtrip DOCX qua Pandoc.",
        }
    ...
    try:
        roundtrip_markdown, summary = normalize_source_markdown(target_file, style_spec=style_spec)
